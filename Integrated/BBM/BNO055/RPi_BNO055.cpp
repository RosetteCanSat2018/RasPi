...
/* written by Hiroki Ando */

/**************************************************************************/
int Adafruit_BNO055::Init()
{
	q = VectorXd(4);
	DCM = MatrixXd(3, 3);
	body = VectorXd(3);
...
void Adafruit_BNO055::SerialInit()
{
	USBHandle = serOpen("/dev/ttyS0",115200,0);
}

int Adafruit_BNO055::AttitudeDeterminate()
{
...
void Adafruit_BNO055::PrintDCM()
{
	cout << DCM(0,0) << "\t" << DCM(0,1) << "\t" << DCM(0,2) << "\t" << DCM(1,0) << "\t" << DCM(1,1) << "\t" << DCM(1,2) << "\t" << DCM(2,0) << "\t" << DCM(2,1) << "\t" << DCM(2,2) << endl;
}

void Adafruit_BNO055::SerialPrint()
{
	while(1){
		if(serDataAvailable(USBHandle) > 0){
			int serial_start = serReadByte(USBHandle); //STAR
			break;
		}
	}
	...
	for(int i = 0; i < 3; i++){
		...
		for(int j = 0; j < 3; j++){
			if(DCM(i,j) > 0){
				s_DCM = to_string(DCM(i,j));
				char c_DCM[9] = {};
				s_DCM.copy(c_DCM, 8);
				serWrite(USBHandle, c_DCM, 9);
			}
			else if(DCM(i,j) < 0){
				s_DCM = to_string(DCM(i,j));
				char c_DCM[9] = {};
				...
				for(int k = 0; k < 9; k++)
				serWriteByte(USBHandle, '0');
			}
		}
	}
}

MatrixXd Adafruit_BNO055::GetDCM()
{
	return DCM;
}
